...
import h5py
from utils.eval_utils_tuning import *
...
parser.add_argument('--task', type=str, choices=['task_1_tumor_vs_normal',  'task_2_tumor_subtyping','custom','custom_1vsall','custom_1vsall_256_20x','custom_1vsall_256_20x_aug'])
...
def main():
    ...
    #            split_dataset = datasets[datasets_id[args.split]]
        #ckpt_paths=["/CLAM/"+ckpt for ckpt in ckpt_path[ckpt_idx]]    
        #model, patient_results, test_error, auc, df  = eval(config,split_dataset, args, ckpt_paths[ckpt_idx])
    results = tune.run(partial(eval,full_dataset=dataset,args=args, ckpt_path=ckpt_paths),resources_per_trial={"cpu": 2, "gpu": 0.5},config=config,scheduler=scheduler, progress_reporter=reporter, num_samples=100)
    best_trial = results.get_best_trial("auc", "max", "last")
    ...
    #print("Best trial final validation ac
    #all_results.append(all_results)
        ...
        #tune.report(auc=auc, accuracy=1-test_error)
        #if not args.eval_features:
        #    df.to_csv(os.path.join(args.save_dir, 'fold_{}.csv'.format(folds[ckpt_idx])), index=False)	
    #tune.report(auc=np.mean(auc),accuracy=np.mean(all_acc))
    #tune.report(auc=auc, accuracy=1-test_error)
...
if __name__ == "__main__":
    ...
    #    device = "cuda:0"
    #config = {
    #    "sampling_random": tune.sample_from(lambda _: 0.1*np.random.randint(0, 10)),
    #    "sampling_neighbors": tune.sample_from(lambda _: 10*np.random.randint(1,10)),
    #    "sampling_random_delta": tune.sample_from(lambda _: 0.01*np.random.randint(0,5)),
    #    "samples_per_epoch":  tune.choice([50, 100, 150, 300])
    #        }
    ...
    if args.profile:
        ...
        profiler.enable()
        #tune.run(main(config),resources_per_trial={"cpu": 4, "gpu": 1},config=config,scheduler=scheduler, progress_reporter=reporter, num_samples=20)
        main()
