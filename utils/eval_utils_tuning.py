...
def eval(config, full_dataset, args, ckpt_path):
    ...
    for ckpt_idx in range(len(ckpt_path)):
        if args.eval_features:
            ...
            if datasets_id[args.split] < 0:
                ...
                datasets = full_dataset.return_splits(from_id=False, csv_path=csv_path)
        ...
        #args.sampling_random_delta=config["sampling_random_delta"]
        #args.samples_per_epoch=config["samples_per_epoch"]
        #args.sampling_epochs=int((900/args.samples_per_epoch)+1)
        ...
        model = initiate_model(args, ckpt_path[ckpt_idx])
        ...
        if args.eval_features:
            ...
            #summary_sampling(model, loader, args)
    ...
    #output_dataframe=output_dataframe.append(pd.DataFrame([output_row]))
...
def summary_eval_features(model,dataset,args):
    ...
    for bag_candidate_idx in range(total):
        ...
        if ((args.samples_per_epoch*args.sampling_epochs)+args.final_sample_size)>len(all_coords):
            ...
            if args.sampling_average:
                    ...
                    if attention_scores[i]>0:
                        for index in indices[sample_idxs[i]][:neighbors]:
                            ...
                            if sampling_weights[index]>0:
                                sampling_weights[index]=(sampling_weights[index]+pow(attention_scores[i],weight_strength))/2
                            else:
                                sampling_weights[index]=pow(attention_scores[i],weight_strength)
                ...
                for i in range(len(sample_idxs)):
                    ...
                    if attention_scores[i]>0:
                        for index in indices[sample_idxs[i]][:neighbors]:
                            #Update the newly found weights
                            sampling_weights[index]=max(sampling_weights[index],pow(attention_scores[i],args.weight_strength))
...
def summary_sampling(model, loader, args):
    ...
    if args.sampling_type=='textural':
        if args.texture_model=='levit_128s':
            texture_dataset =  Generic_MIL_Dataset(csv_path = '/CLAM/dataset_csv/set_all.csv',
                        ...
                        patient_strat= False,
                        ignore=[])
            slide_id_list = list(pd.read_csv('/CLAM/dataset_csv/set_all.csv')['slide_id'])
    ...
    for batch_idx, (data, label,coords,slide_id) in enumerate(loader):
        ...
        if args.sampling_type=='spatial':
            ...
            if args.texture_model=='resnet50':
                ...
                #print(slide_id[0][0])
                #print(full_df)
                #print(full_df['slide_id'])
                #print(full_df['slide_id'][0])
                texture_index=slide_id_list.index(slide_id[0][0])
                levit_features=texture_dataset[texture_index][0]
                #print(len(levit_features[0]))
                #print(levit_features)
                #print(len(data))
        ...
        samples_per_epoch=args.samples_per_epoch
        if (args.samples_per_epoch*args.sampling_epochs+args.final_sample_size)>len(coords):
            #samples_per_epoch=len(coords)
            with torch.no_grad():
                logits, Y_prob, Y_hat, _, results_dict = model(data)
            print("full slide used")
            for _ in range(args.sampling_epochs):
                Y_hats.append(Y_hat)
                labels.append(label)
                Y_probs.append(Y_prob)
                all_logits.append(logits)
            ...
            #print("starting ball_tree")
            nbrs = NearestNeighbors(n_neighbors=args.sampling_neighbors, algorithm='ball_tree').fit(X)
            #print("between statements")
        
            ## only do exhaustive distancing for spatial as it takes too long for textural
            #if args.sampling_type=='spatial':
                    ...
                    for i in range(len(indices)):
                        ...
                        if attention_scores[i]>0:
                            for index in indices[i][:neighbors]:
                                ##Update the newly found weights
                                if sampling_weights[index]>0:
                                    sampling_weights[index]=(sampling_weights[index]+pow(attention_scores[i],args.weight_strength))/2
                                else:
                                    sampling_weights[index]=pow(attention_scores[i],args.weight_strength)
                else:    
                    for i in range(len(indices)):              
                        ##Loop through neighbors of the previously sampled index
                        if attention_scores[i]>0:
        ...
        #print("samples used: ",len(all_sample_idxs))
        acc_logger.log(Y_hat, label)
    ...
    print("all errors: ",all_errors)
    print("all aucs: ",all_aucs)
    #tune.report(auc=auc_score, accuracy=1-test_error)
    #return {"test_error": test_error,"auc_score": auc_score}
    return patient_results, test_error, auc_score, df, acc_logger
